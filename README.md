...
12. In Pimcore SSO configuration, as `Scopes` use `openid`, `profile`, `email` (and optionally [`groups`](https://learn.microsoft.com/en-us/entra/identity/hybrid/connect/how-to-connect-fed-group-claims#add-group-claims-to-tokens-for-saml-applications-using-sso-configuration) if you want to map Entra groups to Pimcore roles)
